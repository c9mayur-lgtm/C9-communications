      ...
      <div className="container mx-auto px-6 md:px-8 max-w-[1240px] relative z-10">
        <div className="max-w-[760px] mx-auto text-center flex flex-col items-center mb-12">
          ...
          <p className="text-gray-400 text-[17px] md:text-[18px] leading-relaxed font-normal" style={{ fontFamily: '"Proxima Nova", sans-serif' }}>
            Australian non-profit organisations are bound by ACNC obligations, the Australian Privacy Act, and increasingly, grant-specific IT and data governance requirements. C9 Defense implements the ACSC Essential 8 framework — giving your organisation documented compliance, continuous monitoring, and auditable security controls.
          </p>
        </div>
          ...
          {[
            {
              ...
              desc: "We map your organisation's IT environment against the ACSC baseline, identify gaps, and deliver a phased remediation roadmap. Ideal for non-profits needing to meet grant compliance or ACNC governance obligations.",
              items: ['Essential 8 implementation and gap assessment', 'Donor and beneficiary data access controls', 'ACNC compliance documentation support'],
              badge: 'C9 DEFENSE ASSESSMENT'
            },
            {
              title: 'Threat Monitoring',
              desc: 'Continuous security monitoring across your environment — detecting ransomware, phishing, and unauthorised access attempts before they become incidents that damage donor trust.',
              items: ['24/7 SOC monitoring', 'Application control enforcement', 'Privileged access governance'],
              badge: 'C9 CONTINUOUS SECURITY'
            },
            {
              title: 'Recovery Assurance',
              desc: 'Immutable backups and tested disaster recovery to protect donor databases, beneficiary records, and grant documentation — with documented recovery targets.',
              items: ['Immutable, air-gapped backups', 'Routine restoration testing', 'Backup and tested disaster recovery'],
            ...
            <div key={idx} className="bg-[#12121A] border border-white/5 rounded-[24px] p-8 md:p-10 shadow-lg flex flex-col h-full hover:bg-[#16161F] transition-colors">
              <h3 className="text-[20px] md:text-[22px] text-white font-bold mb-3" style={{ fontFamily: '"Proxima Nova", sans-serif' }}>{card.title}</h3>
              <p className="text-[15px] md:text-[16px] text-gray-400 mb-8 leading-relaxed font-normal flex-grow" style={{ fontFamily: '"Proxima Nova", sans-serif' }}>{card.desc}</p>
              <div className="flex flex-col mb-10 border-t border-white/5 pt-2">
                {card.items.map((item, index) => (
                  <div key={index} className="flex flex-row items-center gap-4 py-4 border-b border-white/5 px-2">
                    ...
                    <span className="text-[14px] md:text-[15px] font-medium text-gray-300" style={{ fontFamily: '"Proxima Nova", sans-serif' }}>{item}</span>
                  </div>
                ))}
              </div>
              <div className="mt-auto flex items-center gap-4 bg-transparent border border-white/5 rounded-xl px-4 py-3">
                ...
                <span className="text-[11px] font-bold text-gray-400 uppercase tracking-widest" style={{ fontFamily: '"Proxima Nova", sans-serif' }}>{card.badge}</span>
              </div>
            </div>
          ))}
        </div>
