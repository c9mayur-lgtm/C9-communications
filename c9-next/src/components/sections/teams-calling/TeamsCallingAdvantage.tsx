...
const AdvantageItem = ({ title, description, metric, isLast }: { title: string, description: string, metric: string, isLast?: boolean }) => (
  <div className={`p-8 md:p-10 lg:p-12 flex flex-col items-start text-left min-h-[380px] relative group ${!isLast ? 'border-b lg:border-b-0 lg:border-r border-gray-200' : ''}`}>
    ...
    {/* Subtle Background Hover Effect (Using C9 Purple) */}
...
export const TeamsCallingAdvantage = () => {
  return (
        ...
        >
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 w-full">
                <AdvantageItem 
                    ...
                    description="Use your Teams application as a full business phone system with no third-party plug-ins or software required."
                    metric="100%"
                />
                <AdvantageItem 
                    title="Hardware Savings"
                    description="$0 equipment costs. Replace legacy PBX systems and expensive handsets with cloud-native voice."
                    metric="$0"
                />
                <AdvantageItem 
                    title="Operating Costs"
                    description="Reduce your monthly telephony spend by up to 70% while improving your team's mobility."
                    metric="70%"
                />
                <AdvantageItem 
                    title="Service Stability"
                    description="Built on C9's carrier-grade voice core with a dedicated 99.9% availability SLA for Teams traffic."
                    metric="99.9"
                    isLast={true}
                />
            </div>
