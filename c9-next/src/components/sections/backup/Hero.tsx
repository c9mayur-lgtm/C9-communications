        ...
        <div className="grid lg:grid-cols-[60%_40%] gap-12 lg:gap-16 items-center">
          ...
          <div className="flex flex-col items-start max-w-[640px]">
            ...
            <div className="flex flex-wrap items-center gap-4 mt-8 w-full sm:w-auto">
              <Button className="flex-1 sm:flex-none bg-[#5D00D6] hover:bg-[#4d00b3] text-white px-8 h-14 rounded-full font-bold transition-all text-[15px] shadow-lg shadow-purple-900/20" style={{ fontFamily: F }}>
                Assess My Backup Environment <ArrowRight className="ml-2 w-4 h-4" />
              </Button>
              <button
                onClick={scrollToRTO}
                className="flex-1 sm:flex-none flex items-center justify-center text-[#1A1A2E] border border-[#5D00D6]/20 hover:bg-slate-50 px-8 h-14 rounded-full font-bold transition-all text-[15px]"
                style={{ fontFamily: F }}
              >
              ...
              <div key={i} className={`flex justify-between items-center py-4 ${!row.last ? 'border-b border-[#374151]' : ''}`}>
                ...
                <div className="text-right">
                  ...
                  <span
                    ...
                    style={{ fontFamily: F, backgroundColor: row.tagBg, color: row.tagText }}
                  >
                    {row.tag}
                  </span>
                </div>
