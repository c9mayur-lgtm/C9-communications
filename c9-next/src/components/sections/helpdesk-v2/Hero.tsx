        ...
        <AnimatePresence>
          {MESSAGES.slice(0, visible).map((m, i) => (
            <motion.div
              ...
              className={`flex items-end gap-2.5 ${m.from === "user" ? "flex-row-reverse" : "flex-row"}`}
            >
              {/* Avatar */}
              <img
                src={m.from === "user" ? "https://i.pravatar.cc/100?img=33" : "https://i.pravatar.cc/100?img=12"}
                alt={m.from}
                className="w-7 h-7 rounded-full object-cover shrink-0 shadow-sm"
              />
              {/* Bubble */}
              <div
                className={`max-w-[75%] px-4 py-3 rounded-2xl text-[14px] leading-snug font-medium shadow-sm
                  ${m.from === "user"
                    ? "bg-[#5D00D6] text-white rounded-br-sm"
                    : "bg-slate-100 text-[#0c1024] rounded-bl-sm"
                  }`}
              >
                {m.text}
              </div>
          ...
          {/* Typing indicator – show while next message is pending */}
      ...
      <AnimatePresence>
        {visible >= MESSAGES.length && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0 }}
            className="mx-6 mb-4 flex items-center gap-3 bg-emerald-50 border border-emerald-100 rounded-2xl px-5 py-3"
          >
